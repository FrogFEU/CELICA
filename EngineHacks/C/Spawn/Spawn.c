...
void UnitInitFromDefinition(struct Unit* unit, const struct UnitDefinition* uDef)
{
    ...
    {
        if (uDef->charIndex == KeeperUnitID)
        {
            // Old behaviour
            int i;
        ...
        {
            // Load first item normally
            UnitAddItem(unit, MakeNewItem(uDef->items[0]));
            // Set second item to be the matching class core
            unit->items[1] = GetCoreFromClassID(unit, uDef->classIndex);
        }
    }
...
s8 UnitAddItem(struct Unit* unit, int item) {
    ...
    for (i = 0; i < UNIT_ITEM_COUNT; ++i) {
        if (unit->items[i] == 0) {
            unit->items[i] = ConvertWeaponToClassRank(item, GetUnitWeaponRank(unit));
            return TRUE;
        }
    }
